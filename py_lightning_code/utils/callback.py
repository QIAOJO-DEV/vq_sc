...
class ImageLogger(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        # 保存训练集图片
        batch = next(iter(trainer.train_dataloader))  # 获取一个 batch
        self.log_img(pl_module, batch, batch_idx=0, split="train")

    def on_validation_epoch_end(self, trainer, pl_module):
        # 保存验证集图片
        batch = next(iter(trainer.val_dataloaders))
        self.log_img(pl_module, batch, batch_idx=0, split="val")
