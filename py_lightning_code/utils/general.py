...
from py_lightning_code.utils.callback import ImageLogger,SetupCallback
...
def setup_callbacks(exp_config: OmegaConf, config: OmegaConf) -> Tuple[List[Callback], WandbLogger]:
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath="./checkpoints/",
        filename=exp_config.checkpoint_name+"-{epoch:02d}",
        monitor="val/perceptual_loss",
        save_top_k=3,
        save_on_train_epoch_end=True,
        verbose=True,
    )
    os.makedirs(setup_callback.logdir/'wandb', exist_ok=True)
    logger = WandbLogger(save_dir=str(setup_callback.logdir), name=exp_config.name+"_"+str(now), entity="funemo384-jinan-university",
    project="vq_semantic_communication")
...
def resume_callbacks(exp_config: OmegaConf, config: OmegaConf,args) -> Tuple[List[Callback], WandbLogger]:
    basedir = pathlib.Path(args.log_path)
    os.makedirs(basedir, exist_ok=True)
    setup_callback = SetupCallback(config, exp_config, basedir)
    checkpoint_callback = ModelCheckpoint(
        dirpath="./checkpoints/",
    ...
    os.makedirs(setup_callback.logdir/'wandb', exist_ok=True)
    logger = WandbLogger(save_dir=str(args.log_path+'/log'), name=args.wandb_name,id=args.run_id,resume=True,entity="funemo384-jinan-university",project="vq_semantic_communication")
    logger_img_callback = ImageLogger(exp_config.batch_frequency, exp_config.max_images)
